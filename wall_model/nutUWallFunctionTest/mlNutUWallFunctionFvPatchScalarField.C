...
tmp<scalarField> mlNutUWallFunctionFvPatchScalarField::nut() const
{
    ...
    const scalarField& y = turbModel.y().boundaryField()[patchi];
    //const volScalarField& y = turbModel.y();//[patchi];
    //const tmp<scalarField> ty = turbModel.y(patchi);
    //const scalarField& y = ty();
